...
[labelfile,labeldir] = uigetfile({'*.png;*.jpg;*.bmp*;*.mat'});
...
save(fullfile(rootdir,['ResNet_BB_',qdt('Full'),'.mat']),'net','info','options')
...
%% Test Results
testdir = fullfile(rootdir,'testlabelsResNetBB');
mkdir(testdir);
...
segdir = fullfile(rootdir,'SegLabelsResNetBB');
mkdir(segdir);
...
imshow(labeloverlay(im,fullim>0,'Colormap','autumn'));
figure
